...
# ---- vars required by BOTH stacks (login + Plex) ----
require GOOGLE_CLIENT_ID       "Google OAuth client id"
require GOOGLE_CLIENT_SECRET   "Google OAuth client secret"
...
if [[ "$MODE" == "prod" ]]; then
  ...
  require BETTER_AUTH_URL         "public origin, e.g. https://torrents.example.com (binds cookies + OAuth redirect)"
  ...
  url="$(get_env BETTER_AUTH_URL)"
  if [[ -n "$url" && "$url" != https://* ]]; then
    echo "  ${YEL}!${RST} BETTER_AUTH_URL is not https:// ($url) — Google OAuth + secure cookies expect a public https origin"
    warn=1
  fi
else
  # ---- dev: these all have working defaults in docker-compose.dev.yml ----
  optional BETTER_AUTH_SECRET     "has a dev default; fine to leave unset locally"
  optional BETTER_AUTH_URL        "defaults to http://localhost:5173"
